...
if __name__ == '__main__':
  ...
  if not scraper_active:
    ...
    exit()  
  if config_id:
    ret_id = obj_master.obj_mongo_db.recUpdate( 'config' , { 'script_status':'running','ended_at':None,'started_at':datetime.datetime.now() } , { '_id':ObjectId(config_id) } )
  pool = multiprocessing.Pool(processes=max_process)
  temp_date_today = datetime.datetime.now()  
  ...
  for property_url_row in property_url_rows:    
    ...
    while start_date < end_date:        
      ...
      arr_length_stay = obj_master.obj_config.arr_length_stay#[1,2,3,5,7]      
  ...
  #  time.sleep(2)
  if config_id:
      ret_id = obj_master.obj_mongo_db.recUpdate( 'config' , { 'script_status':'end','ended_at':datetime.datetime.now() } , { '_id':ObjectId(config_id) } )
  exit()
